...
?>
	<div class="pull-right social-icon-three clearfix">
                                            ...
                                            <?php if( bunch_set( $options, 'facebook_sharing' ) ) : ?>
                                                <a target="_blank" class="fb" href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>"><span class="fa fa-facebook-f"> </span></a>
                                            <?php endif;?>
    
                                            <?php if( bunch_set( $options, 'twitter_sharing' ) ) : ?>
                                                <a target="_blank" class="tw" href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fa fa-twitter"></span> </a>
                                            <?php endif;?>
    
                                            <?php if( bunch_set( $options, 'pinterest_sharing' ) ) : ?>
                                                <a target="_blank" class="pin" href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fa fa-pinterest-p"></span> </a>
                                            <?php endif;?>
    
                                            <?php if( bunch_set( $options, 'linkedin_sharing' ) ) : ?>
                                                <a target="_blank" class="lin" href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fa fa-linkedin"></span> </a>
                                            <?php endif;?>
                                            
                                        </div>
<?php }
function bunch_like_us($PostID = '')
{ 
$like = get_post_meta( $PostID, '_jolly_like_it', true );
?>
<a href="javascript:void(0);" class="jolly_like_it" data-id="<?php $PostID; ?>"><i class="zmdi zmdi-favorite-outline"></i>&nbsp; <?php echo (int)$like;?> <?php esc_html_e( 'Likes', 'minipo' ); ?></a>
<?php }?>
<?php /*
<?php if(function_exists('bunch_like_us')) echo wp_kses_post(bunch_like_us(get_the_id()));?>
*/?>